...
- **주석**: `*.h/*.hpp`(외부용)는 **영어 + Doxygen 강제**(공개 API 계약만). `*.c/*.cpp`(내부용)는 **한국어**, Doxygen 허용 — 알고리즘 상세·설계 의도·트레이드오프까지 적어도 된다.
- **타입**: raw stdint 대신 LBX 별칭(`i32_t`, `u32_t`, `f32_t`, `b8_t`, `var_t`, `fourcc_t`, `LBX_HANDLE`).
